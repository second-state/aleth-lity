...
size_t gas_result_length(eni_return_data p) {
    ...
    (void)p;  // -Werror=unused-parameter
...
uint64_t fork_gas(void* f, char *argsText, int* status) {
    uint64_t* ret = (uint64_t*)fork_call(gas_result_length, eni_gas_executor, (eni_function)f, argsText, status);
...
*/
    while (true) {
        ...
        int nread = read(pfd, (char*)ret + ret_len, ret_cap - ret_len);
        if (nread > 0) { // Success
            ret_len += nread;
            continue;
        }
